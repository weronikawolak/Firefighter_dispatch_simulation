...
class FireStrategy(IStrategy):
    """
    Strategy for handling fire events.
    """

    def required_vehicles(self):
        return 3  # Number of vehicles needed for a fire event

    def execute(self, unit, event):
        needed_vehicles = self.required_vehicles()
        logger.info(f"🔥 [Fire] Event detected. Dispatching {needed_vehicles} vehicles from unit: {unit.name}")
    ...
    def _dispatch_and_simulate(self, unit, count, event):
        with unit.lock:
            ...
            if len(free_vehicles) >= count:
                ...
                logger.info(f"🚒 Dispatched {len(dispatched)} vehicles for fire event ID: {event.id}.")
            else:
    ...
    def _simulate_action(self, unit, vehicles, event):
        ...
        logger.info(f"🚦 Vehicles en route to fire event ID: {event.id}. Travel time: {travel_time}s.")
        time.sleep(travel_time)

        # Check for false alarm
        if random.random() < 0.05:  # 5% chance of false alarm
            logger.info(f"🚨 False alarm for fire event ID: {event.id}. Vehicles are returning to base.")
            ...
            logger.info(f"✅ Vehicles returned to base after false alarm for fire event ID: {event.id}.")
            return

        action_time = random.randint(5, 25)
        logger.info(f"🔥 Handling fire event ID: {event.id}. Estimated duration: {action_time}s.")
        time.sleep(action_time)

        return_time = random.randint(0, 3)
        logger.info(f"🔙 Vehicles returning from fire event ID: {event.id}. Return time: {return_time}s.")
        time.sleep(return_time)

        with unit.lock:
            for vehicle in vehicles:
                vehicle.set_state(FreeState())
        logger.info(f"✅ Vehicles returned to base after handling fire event ID: {event.id}.")